...
def random_mask(im_shape, ratio=1, mask_full_image=False):
    ...
    size = (secrets.SystemRandom().randint(0, int(im_shape[0] * ratio)), secrets.SystemRandom().randint(0, int(im_shape[1] * ratio)))
    # use this to always mask the whole image
    if mask_full_image:
        size = (int(im_shape[0] * ratio), int(im_shape[1] * ratio))
    limits = (im_shape[0] - size[0] // 2, im_shape[1] - size[1] // 2)
    center = (secrets.SystemRandom().randint(size[0] // 2, limits[0]), secrets.SystemRandom().randint(size[1] // 2, limits[1]))
    draw_type = secrets.SystemRandom().randint(0, 1)
